...
class DeclinedShift extends Component
{
    constructor(props) {
        super(...arguments);
        ...
        prevDate = null;
        this.state = {
            dataSource: ds.cloneWithRows(props.declinedData || [])
        };
    }

    componentWillReceiveProps(nextProps) {
        if (nextProps.declinedData) {
            let ds = new ListView.DataSource({rowHasChanged: (r1, r2) => r1 !== r2});
            // prevDate = nextProps.workedData[0].startTime;
            this.state = {
                dataSource: ds.cloneWithRows(nextProps.declinedData || [])
            };
        }
    }
    ...
    renderRow = (item) => {
        ...
        // calculate wage (copied from WorkedShift.js)
        ...
        // countdown calculations (copied from ScheduleCardComponent.js)
        let phoneTreeHasRun = !!item.callMade;
        let bidding_end = moment(item.biddingPeriodExpiration) || moment().format();
        ...
        } else if (!phoneTreeHasRun) {
            // TODO: disallow accepting shifts after countdown runs out, unless they are in instant pickup
            // return null;
        }
        ...
        // determine status of shift
        let status = moment(item.startTime).isBefore(moment()) ? 'PAST' : 'OPEN';
        let color = (status === 'OPEN') ? '#E33820' : '#4A4A4A';
        let icon = (status === 'OPEN') ? require('../../assets/icons/open.png') : require('../../assets/icons/history.png');
        return (
                            ...
                            <View style={{flex: 1, width: "95%", padding: 3}}>
                                <View style={{flex: 0.6, justifyContent: 'space-between', flexDirection: 'row'}}>
                                    <View style={{margin: 5, width: '80%'}}>
                                        ...
                                        <Text numberOfLines={1} ellipsizeMode="tail" style={styles.h4BlackLatoBlack}>{item.brandName}
                                            | {item.workplaceName}</Text>
                                        <Text style={styles.h4BlackLato}>Declined</Text>
                                    </View>
                                ...
                                <View style={{flex: 0.4, flexDirection: 'row', alignItems: 'center', margin: 5, marginTop: 45}}>
                                    <View style={{marginRight: 8}}>
                                        {this.renderData(moment(item.startTime).format('h:mm').toUpperCase(), moment(item.startTime).format('a').toUpperCase())}
                                    </View>
                                    <View>
                                        {this.renderData(moment(item.endTime).format('h:mm').toUpperCase(), moment(item.endTime).format('a').toUpperCase())}
                                    </View>
    ...
    render(){
        if (this.props.declinedData === null) {
            return (<View>
                <Text> Loading... </Text>
            </View>);
        };
        if (this.props.declinedData.length === 0) {
            return (<View>
